...
import { getSupportTicketDetails, getSupportTicketMessages } from '../../../services/complaintsService';
...
export const useSupportTicketChat = ({ onAfterMessage }) => {
  ...
  const sendSupportMessage = async (event) => {
    ...
    try {
      ...
      setChatError(toClientError(error, 'Failed to send support message.'));
    } finally {
      setChatSending(false);
    }
  };
