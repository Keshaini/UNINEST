...
import { getStudentTicketDetails, getStudentTicketMessages } from '../../../services/complaintsService';
...
export const useTicketModal = ({ currentStudentId, onLoadMyComplaints, fallbackStudentName }) => {
  ...
  const { sendSocketMessage } = useTicketChatSocket({
  ...
  const handleSendChatMessage = async (event) => {
    ...
    setChatSending(true);
    try {
      const sentMessage = await sendSocketMessage({
      ...
      setTicketModalError(toClientError(error, 'Failed to send message.'));
    } finally {
      setChatSending(false);
    }
  };
