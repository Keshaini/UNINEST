...
function Reports() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0B1120] flex flex-col items-center justify-center p-10">
  ...
  return (
    <div className="min-h-screen bg-[#0B1120] text-slate-300 p-6 lg:p-10 animate-fade-in selection:bg-indigo-500/30">
      <div className="max-w-[1600px] mx-auto space-y-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
            ...
            <div className="lg:col-span-12 grid grid-cols-1 xl:grid-cols-2 gap-8 lg:gap-12">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                            {paymentMethodData.map((item, idx) => (
                                <div key={idx} className="p-4 bg-slate-950 rounded-2xl border border-slate-800/50 flex flex-col items-center gap-2 group hover:border-indigo-500/50 transition-all">
                                    <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">{item.name}</span>
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-[3rem] p-4 lg:p-10 shadow-3xl">
                    ...
                    <div className="overflow-x-auto px-6 lg:p-0">
                        <table className="w-full text-left">
                            <thead className="bg-slate-950/50 border-b border-slate-800">
                                <tr>
                                    <th className="px-6 py-4 text-[9px] font-black text-slate-500 uppercase tracking-widest">Elite Index</th>
                                    ...
                                    <tr key={index} className="group hover:bg-slate-950/40 transition-all">
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-4">
                                                <span className="w-8 h-8 rounded-lg bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-[10px] font-black text-indigo-400">0{index + 1}</span>
                                                <span className="text-sm font-black text-slate-200 group-hover:text-white transition-colors">{student._id}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5 text-right font-black text-slate-300 transition-colors">{formatCurrency(student.totalPaid)}</td>
                                        <td className="px-6 py-5 text-right text-[10px] font-black text-slate-600 uppercase tracking-widest group-hover:text-indigo-400">{student.paymentCount} Trans</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
    ...
    <div className="space-y-3">
        ...
        <div className="relative group">
            ...
            <input
                ...
                className="w-full bg-slate-950/30 border border-slate-800/50 rounded-2xl pl-16 pr-6 py-4 text-sm font-black text-white outline-none focus:border-indigo-500/40 focus:bg-slate-950 transition-all cursor-pointer"
            />
        </div>
    </div>
);
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 shadow-2xl group hover:border-slate-700 transition-all relative overflow-hidden">
            ...
            <div className="flex flex-col gap-6 relative z-10">
                <div className="flex items-center justify-between">
                    ...
                    <span className={`text-[9px] font-black px-3 py-1 bg-slate-950 rounded-lg border border-slate-800/50 uppercase tracking-widest ${trend.startsWith('+') ? 'text-emerald-500' : trend.startsWith('-') ? 'text-rose-500' : 'text-slate-500'}`}>
                        {trend}
                    </span>
                </div>
                <div>
