...
export const useComplaintActions = ({
  ...
  const onSubmitComplaint = useCallback(
    async (event) => {
      ...
      try {
        ...
        setFormData({ ...INITIAL_FORM, studentId: submittedStudentId });
    ...
    [formData, currentStudentId, setCurrentStudentId, setFormData, setFeedback, isSupportRoute, supportPage, loadStudentComplaints, loadSupportDashboard]
  );
  ...
  const onDeleteComplaint = useCallback(
    async (complaintId) => {
      // Removed legacy window.confirm. Confirmation should be handled at the UI layer.
      try {
